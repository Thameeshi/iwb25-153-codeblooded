...
function generateRecommendation(CloudResource resource, int score) returns AISuggestion {
    ...
    string confidence;
    if score >= 5 {
        rec = "Your " + resource.resourceType + " `" + resource.id + "` is mostly idle. Consider downsizing or stopping it to save ~$" + 
              resource.costPerMonth.toString() + "/month.";
        confidence = "High";
    } else if score >= 3 {
        rec = "Monitor " + resource.resourceType + " `" + resource.id + "`; usage may need optimization soon.";
        confidence = "Medium";
    } else {
        rec = resource.resourceType + " `" + resource.id + "` is performing optimally.";
        confidence = "Low";
    }
