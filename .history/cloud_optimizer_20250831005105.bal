...
<a href="/resources">All Resources</a>
    ...
    resource function get report(http:Request req) returns http:Response {
        ...
        string provider = "AWS";
        if queryParams["provider"] is string[] p && p.length() > 0 {
            provider = p[0];
        }
        ...
        if queryParams["vm"] is string[] v && v.length() > 0 {
            vm = check float:fromString(v[0]);
        }
        if queryParams["storage"] is string[] s && s.length() > 0 {
            storage = check float:fromString(s[0]);
        }
        if queryParams["network"] is string[] n && n.length() > 0 {
            network = check float:fromString(n[0]);
        }
...
<a href="/">Back to Home</a>
    ...
    resource function get ai-suggestions() returns http:Response {
        ...
        foreach var r in resources {
            ...
            suggestions.push(generateRecommendation(r, score));
        }
...
<tr><th>Resource ID</th><th>Recommendation</th><th>Confidence</th></tr>`;
        ...
        foreach var rec in suggestions {
            html += string `<tr>
