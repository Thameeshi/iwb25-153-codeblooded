...
.login-modal { position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: rgba(0,0,0,0.9); display: flex; align-items: center; justify-content: center; z-index: 10000; }
.login-content { background: white; padding: 3rem; border-radius: 20px; text-align: center; max-width: 400px; box-shadow: 0 20px 60px rgba(0,0,0,0.3); position: relative; z-index: 10001; }
.login-content h2 { margin-bottom: 1rem; color: #333; }
.login-content p { margin-bottom: 2rem; color: #666; }
.footer { background: rgba(0,0,0,0.2); color: white; text-align: center; padding: 3rem 0; margin-top: 4rem; position: relative; z-index: 50; }
...
.footer-social a { color: white; opacity: 0.8; text-decoration: none; }
.footer-bottom { border-top: 1px solid rgba(255,255,255,0.2); padding-top: 2rem; opacity: 0.8; }
...
<div class="container">
    ...
    <div id="home" class="page active">
        ...
        <div class="features">
            <div class="feature"><div class="feature-icon">📊</div><h3>Real-time Analysis</h3><p>Get instant cost breakdowns across all major cloud providers</p></div>
            <div class="feature"><div class="feature-icon">💰</div><h3>Cost Optimization</h3><p>Find the most cost-effective cloud solution for your needs</p></div>
            <div class="feature"><div class="feature-icon">📈</div><h3>Visual Reports</h3><p>Beautiful charts and downloadable reports for easy analysis</p></div>
        </div>
    </div>
    ...
    <div id="calculator" class="page">
        <div class="calc-card">
            <div class="calc-header"><h2>Cloud Cost Calculator</h2><p>Enter your requirements to get instant cost estimates</p></div>
            <div class="calc-grid">
                <div>
                    ...
                    <div class="form-group"><label class="form-label">VM Hours/Month</label><input id="vmHours" type="number" class="form-input" value="744" oninput="calculate()"></div>
                    <div class="form-group"><label class="form-label">Storage (GB)</label><input id="storage" type="number" class="form-input" value="100" oninput="calculate()"></div>
                    <div class="form-group"><label class="form-label">Network Transfer (GB)</label><input id="network" type="number" class="form-input" value="50" oninput="calculate()"></div>
                </div>
                <div>
                    <h3>Cost Breakdown</h3>
                    <div class="results">
                        <div class="result-item"><span>VM Cost:</span><span id="vmCost">$37.20</span></div>
                        <div class="result-item"><span>Storage Cost:</span><span id="storageCost">$1.00</span></div>
                        <div class="result-item"><span>Network Cost:</span><span id="networkCost">$1.00</span></div>
    ...
    <div id="about" class="page">
        <div class="calc-card">
            ...
            <p style="line-height:1.8;color:#666;margin-top:1rem;">CloudOptimizer Pro helps businesses make informed decisions about cloud infrastructure costs. We provide real-time pricing comparisons across AWS, Azure, and Google Cloud to help you optimize your cloud spending and choose the most cost-effective solution for your needs.</p>
        </div>
    </div>
    ...
    <div id="contact" class="page">
        <div class="calc-card">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:2rem;margin-top:2rem;">
                <div><h3>Get in Touch</h3><p style="color:#666;margin:1rem 0;">Have questions about cloud optimization? We're here to help!</p>
                    <div style="margin:1rem 0;"><strong>Email:</strong> support@cloudoptimizer.pro<br><strong>Phone:</strong> +1 (555) 123-4567<br><strong>Address:</strong> 123 Cloud Street, Tech City, TC 12345</div>
                </div>
                <div><h3>Quick Contact</h3>
    ...
    <div id="dashboard" class="page">
        <div class="calc-card">
            <h2>Dashboard</h2>
            <iframe src="http://localhost:8081/" style="width:100%; height:600px; border:none; border-radius:10px;"></iframe>
        </div>
    </div>
...
<footer class="footer">
    <div class="footer-content">
        <div class="footer-grid">
            <div class="footer-section"><h3>CloudOptimizer Pro</h3><p>Your trusted partner for cloud cost optimization and multi-cloud management.</p></div>
            <div class="footer-section"><h4>Quick Links</h4><div class="footer-links">
            ...
            <div class="footer-section"><h4>Cloud Providers</h4><div><p>Amazon Web Services</p><p>Microsoft Azure</p><p>Google Cloud Platform</p></div></div>
        ...
        <div class="footer-bottom">&copy; 2025 CloudOptimizer Pro. All rights reserved.</div>
    </div>
</footer>
...
function calculate() {
    ...
    let vmHours = parseFloat(document.getElementById('vmHours').value);
    let storage = parseFloat(document.getElementById('storage').value);
    let network = parseFloat(document.getElementById('network').value);

    let pricing = { "AWS": {"vm":0.05,"storage":0.01,"network":0.02},
                    "Azure":{"vm":0.045,"storage":0.012,"network":0.018},
                    "Google":{"vm":0.048,"storage":0.011,"network":0.019} }[provider];

    let vmCost = vmHours * pricing.vm;
    ...
    renderChart([vmCost, storageCost, networkCost], ['VM', 'Storage', 'Network']);
}
...
function renderChart(data, labels) {
    ...
    chart = new Chart(ctx, {
        type:'doughnut',
        data:{labels:labels, datasets:[{data:data, backgroundColor:['#667eea','#764ba2','#ff6b6b']}]}
    });
}
