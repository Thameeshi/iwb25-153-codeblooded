...
service / on new http:Listener(8081) {
    ...
    resource function get .() returns http:Response {
        ...
        html += "<script>"
                ...
                + "let rec = conf === 'High' ? `Your ${res.resourceType} \`${res.id}\` is mostly idle. Consider downsizing or stopping it to save ~$${res.costPerMonth}/month.`"
                + ": conf === 'Medium' ? `Monitor ${res.resourceType} \`${res.id}\`; usage may need optimization soon.`"
                + ": `${res.resourceType} \`${res.id}\` is performing optimally.`;"
                + "let li = document.createElement('li');"
                + "li.innerHTML = `<span class='resource-type'>${res.resourceType} \`${res.id}\`</span><br>CPU: ${res.cpuUsage}% | Storage: ${res.storageUsage} GB | Cost: $${res.costPerMonth}/month"
                + `<div class='recommendation ${conf.toLowerCase()}'>Recommendation: ${rec} (Confidence: ${conf})</div>`;"
                + "ul.appendChild(li);"
                + "});"
                + "}"
