...
service / on new http:Listener(8081) {
    ...
    resource function get .() returns http:Response {
        string html = "<html><body><h1>CloudOptimizer Pro</h1>" +
                      "<ul>" +
                      "<li><a href='/ai-suggestions'>AI Cost Recommendations</a></li>" +
                      "<li><a href='/report?provider=AWS&vm=100&storage=500&network=200'>Sample Cost Report</a></li>" +
                      "<li><a href='/resources'>List All Resources</a></li>" +
                      "</ul></body></html>";
...
Generated: ${time:utcToString(time:utcNow())}`;
    ...
    // AI Suggestions
    resource function get ai\-suggestions(http:Request req) returns json {
        map<string[]> params = req.getQueryParams();
        ...
        foreach CloudResource res in resources {
            if typeFilter is string[] && typeFilter.length() > 0 {
                if res.resourceType != typeFilter[0] {
                    continue;
                }
            }
