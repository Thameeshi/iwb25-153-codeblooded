...
// Simulated cloud resources for AI
type CloudResource record {
    string id;
    string type; // EC2, S3, etc.
    float cpuUsage; // in %
    float storageUsage; // in %
    float costPerMonth;
};
...
function getAIScore(CloudResource resource) returns int {
    if resource.type == "EC2" {
        if resource.cpuUsage < 10.0 {
            return 5; // High priority
        } else if resource.cpuUsage < 50.0 {
            return 3; // Medium
        } else {
            return 1; // Low
        }
    } else if resource.type == "S3" {
        if resource.storageUsage > 70.0 {
            return 5;
        } else if resource.storageUsage > 40.0 {
            return 3;
        } else {
            return 1;
        }
...
// Convert AI score to recommendation text
...
service / on new http:Listener(8080) {
    // Home / calculator / report endpoints remain same as before
    resource function get .() returns http:Response {
        // Your existing HTML content for home, calculator, etc.
        http:Response res = new;
        res.setPayload("<html><body><h1>CloudOptimizer Pro</h1><p>Use /ai-suggestions for AI cost recommendations.</p></body></html>");
    ...
    resource function get report(http:Request req) returns http:Response {
        ...
        if storageParam is string[] && storageParam.length() > 0 {
            ...
            if storageResult is float stVal {
                storage = stVal;
            }
        }
        ...
        if networkParam is string[] && networkParam.length() > 0 {
            ...
            if networkResult is float nwVal {
                network = nwVal;
            }
        }
